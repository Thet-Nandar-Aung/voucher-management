...
public class FeedControllerTest {
	...
	private static User user = new User("1", "admin12345@gmail.com", "Admin", "Pwd@123", RoleType.MERCHANT, null, null,
			true, null,
	...
	private static Feed feed1 = new Feed("1", campaign1, false, false, null, user, LocalDateTime.now());
	private static Feed feed2 = new Feed("2", campaign2, false, false, null, user, LocalDateTime.now());
	...
	void testGetAllByEmail() throws Exception {
		...
		Map<Long, List<FeedDTO>> mockFeedMap = new HashMap<>();
		mockFeedMap.put(0L, mockFeeds);
		Mockito.when(feedService.findAllFeedsByEmail(user.getEmail(), pageable)).thenReturn(mockFeedMap);
		mockMvc.perform(MockMvcRequestBuilders.post("/api/feed/getAllByEmail").param("page", "0").param("size", "10")
				.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(user)))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.message").value("Successfully get all feeds for email: " + user.getEmail()))
				.andDo(print());
	}
	...
	void testUpdateReadStatusById() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/feed/updateReadStatusById")
				.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(feed1)))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.success").value(true)).andDo(print());
	}
